...
class CNNEvaluation(object):
    ...
    def __call__(self, net_lists):
        ...
        for i in np.arange(0, len(net_lists), self.gpu_num):
            ...
            arg_data = [(cnn_eval,self.cnf, net_lists[i + j], j, self.epoch_num, self.batchsize, self.dataset, self.reduced, self.verbose,
        ...
        return evaluations      # 目前の目標はここの意味する内容を理解すること(loss , paramaters) ここは個体ごとにあるから２つのときもある
